...
const parallelModuleDev: DagGraph = {
  ...graph(
  ...
  variables: [{ key: 'task', label: '开发对象（做什么项目/模块）', required: false }],
};

/** 场景二：标准化工程流水线固化复用 */
const standardDevFlow: DagGraph = {
  ...graph(
    ...
    [
      ...
      agent(
        'code',
        '功能编码',
        '目标功能：{{feature}}（为空则自选一个与项目定位相称的核心功能）。基于初始化结果实现它：{{init.artifact.summary}}。要求：可运行、带最小 CLI 或入口、代码遵循项目 lint 规范。完成后写结果文件并列出创建/修改的文件。',
        { retryCount: 1 },
      ),
      agent(
        'verify',
  ...
  variables: [{ key: 'feature', label: '要实现的核心功能', required: false }],
};

/** 场景三：高风险操作人工审批 */
const riskApprovalFlow: DagGraph = {
  ...graph(
    ...
    [
      ...
      agent(
        'audit',
        '现状审计',
        '审计当前工作目录项目：列出依赖清单及每个依赖的当前版本与已知风险（可读 lock 文件/package manifest）。本次升级范围：{{upgrade_scope}}（为空则按全部可升级依赖评估）。只读不写，结论写入结果文件。',
        { onFail: 'abort' },
      ),
      agent(
        'upgrade',
  ...
  variables: [{ key: 'upgrade_scope', label: '升级范围（目标依赖或「全部」）', required: false }],
};

/** 场景四：多步骤 Bug 调试修复流水线 */
const bugFixPipeline: DagGraph = {
  ...graph(
    ...
    [
      ...
      agent(
        'repro',
        '问题复现',
        '缺陷输入：{{bug_report}}（可为现象描述/复现步骤/Issue 链接；为空则自行物色一个典型缺陷）。在当前工作目录：定位项目入口，理解项目用途，构造一个可稳定复现的缺陷场景（若无真实 bug，则人为注入一个典型缺陷再复现）。复现步骤与现象写入结果文件。',
        { retryCount: 1 },
      ),
      agent(
        'trace',
  ...
  variables: [{ key: 'bug_report', label: '缺陷现象 / 复现步骤 / Issue 链接', required: false }],
};

/** 场景五：多 Agent 角色化团队协作 */
const roleTeamReview: DagGraph = {
  ...graph(
    ...
    [
      ...
      agent(
        'dev',
        '开发工程师',
        '你是开发工程师。在当前工作目录实现指定功能：{{feature}}（为空则自选命令行 TODO 工具即可），要求功能完整可运行。只做实现，不写测试不写文档。交付说明写入结果文件。',
  ...
  variables: [{ key: 'feature', label: '团队要开发的功能', required: false }],
};

/** 场景六：双路调研对比决策 */
const researchCompare: DagGraph = {
  ...graph(
    ...
    [
      ...
      agent(
        'planA',
        '调研路线A',
        '你是技术调研员，只调研「方案A：{{plan_a}}」路线：在本目录写 research/plan-a.md，评估维度为 开发成本/性能/运维复杂度/扩展性，各给评分(1-5)与理由。不比较其他方案，只深挖本路线。结论摘要写入结果文件。',
        { cwd: 'research/a', onFail: 'continue' },
      ),
      agent(
        'planB',
        '调研路线B',
        '你是技术调研员，只调研「方案B：{{plan_b}}」路线：在本目录写 research/plan-b.md，评估维度同开发成本/性能/运维复杂度/扩展性，各给评分(1-5)与理由。不比较其他方案，只深挖本路线。结论摘要写入结果文件。',
  ...
  variables: [
    { key: 'plan_a', label: '方案A（调研对象）', default: '本地 SQLite + 单文件部署' },
    { key: 'plan_b', label: '方案B（调研对象）', default: 'Postgres + Docker Compose 部署' },
  ],
};
  ...
  ...graph(
    ...
    [
      ...
      agent(
        'explore',
        '探索与评估',
        '项目根目录下探索该项目：读 README、docs、目录结构与关键代码，评估「功能成熟度」（已实现哪些能力/缺失哪些能力/明显的体验或工程缺口），围绕用户给的需求描述判断该做什么。用户需求描述：{{brief}}（为空则纯靠探索发现缺口）。',
