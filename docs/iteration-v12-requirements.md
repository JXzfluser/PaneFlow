...
### 追加波（2026-09-23，用户令「规划全部、多 agent 一起实现」，双 agent 并行+收口人复验）

| 片 | commit | 落点与实机 |
|---|---|---|
| 模板片（S1a 兑现） | `35e0d51` | 内置模板 GitHub 写调用携 `"runId":"{{run_id}}"`（受理路草稿 JSON+对齐路 curl body 两处措辞，run_id 引擎内置变量零新机制）；gh 直连路不经 server 如实不动；dataDir 仪式已走（旧副本 .bak-v12tpl→重启重播→新措辞落盘验证 1 处/模板）。server 485 全绿 |
| C4 第一夜执行包 | `88d5866` | `scripts/c4-night1.sh` dry/night1/collect 三子命令固化开夜序列；护栏=不代批/dataDir 只读探测/失败即停；模板片生效门实测判别力（旧 0/新 1）；踩修探测式转义 bug 入注。distill 判死/三夜语料时序/选题 #8 落 v11 文档 `f2b62cb` |
...
**门控位现状**：C4 选题已定（#8/#9/#10）待开跑——执行包已固化 `./scripts/c4-night1.sh`，模板片 dataDir 仪式已走，万事俱备只烧配额；T1 等 C4 人肉跑通；V3 等 C4 假绿证据；D4 等实机复现。master 领先 origin/main 9 提交（含本表两轮），**未推**（等明示）。
